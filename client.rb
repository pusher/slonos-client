...
chan.bind('add') do |data|
  ...
  speaker.add_spotify_to_queue({ :id => decoded['id'] + '?sid=9&amp;flags=0', :title => decoded['title'] }) # No idea why we need that string, but otherwise we get a 800 response from Sonos
